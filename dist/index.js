...
export default definePlugin({
    ...
    patchGuildIcons() {
        ...
        const updateGuildClickHandlers = () => {
            ...
            guildButtons.forEach((btn) => {
                if (btn._mslPatched)
                    return;
                btn._mslPatched = true;
                btn.addEventListener("click", (e) => {
                    if (!self.selectMode)
                        return;
                    const guildId = self.extractGuildIdFromElement(btn);
                    if (!guildId)
                        return;
                    e.stopPropagation();
                    e.preventDefault();
    ...
    cleanup() {
        ...
        document
            ...
            .forEach((el) => delete el._mslPatched);
    }
});
