...
export default definePlugin({
    ...
    selectedGuilds: new Map<string, SelectedGuild>(),
    selectMode: false,
    isLeaving: false,
    ...
    toggleSelectMode(toggleBtn: HTMLElement, leaveBtn: HTMLElement, counter: HTMLElement) {
    ...
    patchGuildIcons() {
        ...
        const updateGuildClickHandlers = () => {
            ...
            guildButtons.forEach((btn: Element) => {
                if (btn.classList.contains("msl-patched")) return;
                btn.classList.add("msl-patched");

                btn.addEventListener("click", (e: Event) => {
                    if (!this.selectMode) return;

                    const guildId = this.extractGuildIdFromElement(btn as HTMLElement);
                    if (!guildId) return;
                    ...
                    this.updateGuildHighlight(btn as HTMLElement);
                    this.updateCounterDisplay(document.querySelector(".msl-counter") as HTMLElement);
                }, true);
            });
        };
    ...
    updateCounterDisplay(counter: HTMLElement | null) {
        if (!counter) return;
        counter.textContent = this.selectedGuilds.size.toString();
    },

    async leaveSelectedGuilds(leaveBtn: HTMLElement, counter: HTMLElement, toggleBtn: HTMLElement) {
        ...
        // Filter guilds
        const ownedGuilds: SelectedGuild[] = [];
        const guildsToLeave: SelectedGuild[] = [];
